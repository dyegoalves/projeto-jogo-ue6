var idAtual, limite = 35, visitados, largura = 0, altura = 0, unityObj, isFullScreen = 0;
var tamTituloH1 = 18, tamTituloH2 = 15, tamFonte = 13, tamLegenda = 9, temMenu;
var masteryScore, overlayDlg = null, vars; //xx, ludos_pasta = '../../scorm_export/';
//xx var ludos_tipo_scorm = 2; // -1=webcursos novo; 0=sem scorm; 1=lê do BD e salva no scorm; 2=lê e salva no scorm
var skinCss, skinVet = {
  'bgMain': '#375a64', 'txtMain': '#fff',
  'bgRealce': '#467582', 'txtRealce': '#fff',
  'bgAtivo': '#467582', 'txtAtivo': '#fff',
  'bgMenuOff': '#ececec', 'txtMenuOff': '#fff',
  'bgMenuOn': '#467582', 'txtMenuOn': '#fff', 'bgMenuMarcadorVisitado': '#f8c301',
  'bgDestaque': '#feebb2', 'txtDestaque': '#000', 'titDestaque': '#375a64'
};
...
function set_html_page(tit_curso) {
  ...
  if ((api = getAPIHandle()) != null)
    loadPage();
  /*xx  try {
      var win = (top.window.opener==null ? top : top.window.opener);
      if(typeof win.SPA=='object'&& typeof win.SPA.cfg=='object') { // webcursos novo
        ludos_tipo_scorm = -1;
        ludos_pasta = win.location.href.replace('http://','').replace('https://','');
        ludos_pasta = ludos_pasta.substring(ludos_pasta.indexOf('/'),
         ludos_pasta.lastIndexOf('/')+1);
      } 
    } catch(e) {
    }*/
  temMenu = ($('#menu').length > 0 ? true : false);
  set_page_skin();
  document.getElementById('content').innerHTML = getHtmlTopo(tit_curso, temMenu, true)
  ...
  atualizaVisitados(0);
  if (!temMenu) {
    api.LMSSetValue('cmi.core.score.raw', '100.00');
    api.LMSCommit('');
  }
  load_js_file('../../lib/mtree.js', true, function () {
    $('#menu').perfectScrollbar({
      wheelSpeed: 5, wheelPropagation: false,
    ...
    /*xx   load_css_file(ludos_pasta+'desafios.css');
...
function getHtmlTopo(tit, menu, temTelaCheia) {
  return '<section id="topo">\n'
    + '  <div>'
    + (!menu ? '' : '    <img alt="Menu Retrátil" title="Menu Retrátil" src="../../images/menu_tree.png" onclick="toggleMenu(this)">\n')
    ...
    + (!temTelaCheia ? '' : '    <img alt="Tela Cheia" title="Tela Cheia" src="../../images/icone_fullscreen.png" onclick="toggleFullScreen(document.documentElement)">\n')
    + '  <\/div>\n'
    + '<\/section>\n';
}
...
function atualizaVisitados(id) {
  ...
  var x, vet, obj;
  if (temMenu) {
    vet = $('a[href^="javascript:go("]');
    for (x = 0; x < vet.length; x++) {
      obj = $(vet[x]);
      if (!obj.hasClass('visitado'))
        break;
    }
    if (x == vet.length) // todos tem imagem (nao deu break)
      api.LMSSetValue('cmi.core.score.raw', '100.00');
  }
  if (id > 0)
    api.LMSCommit('');
...
function winResize() {
  ...
  if (largura != w || altura != h) {
    ...
    $('#content').css({
      'height': (altura - 22) + 'px',
      'width': (largura - ($('#menu').length == 0 || $('#menu').hasClass('invisivel') ? 20 : 250)) + 'px'
    });
    ...
    setTimeout(function () { // o timeout espera a página ir para a direita
      $.ajax({
        ...
        success: function (data) {
          ...
          bindPlugins();
          try {
            $('#conteudoScroll').perfectScrollbar({
              wheelSpeed: 5, wheelPropagation: false,
              suppressScrollX: true
            });
          } catch (err) {
...
function bindPlugins() { // pode ser usado nos popups
  ...
  if (objs.length > 0) {
    objs.turn({
      autoCenter: false, display: 'double', acceleration: true,
  ...
  $('div[id^=flash_]').each(function () {
    ...
    this.innerHTML = flash(this.innerHTML, '', this.style.width, this.style.height);
  });
  ...
  $('.unity_obj').each(function () {
    ...
    try {
      unityObj = new UnityObject2({
        ...
        params: {
          backgroundcolor: bc, bordercolor: bc, textcolor: 'FFFFFF', logoimage: '../../images/logo.png',
  ...
  // cria os jogos
  /*xx  $('[id^=layer_jogo]').each(function() {
...
//Troca video  no id=lista
function trocarvideo(ev) {
  var video1 = "../videos/uc3_ue1_01.mp4"
  var video2 = "../videos/uc3_ue1_02.mp4"
  var playvideo1 = document.getElementById("playervideo1");
  var playvideo2 = document.getElementById("playervideo2");
  if (ev == 1) {
    playvideo1.src = video1;
  } else {
    playvideo2.src = video2;
  }
}
